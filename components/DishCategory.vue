<template>
    ...
    >
        <div ref="containerEl" class="container mx-auto min-h-[10rem] px-4">
            <h1 class="my-8 text-2xl font-medium uppercase text-black">{{ props.category.name }}</h1>
            <Transition
                name="fade"
                mode="out-in"
            >
...
const { data, status } = await useAsyncData<{dishes: Dish[]}>(
    ...
    () => ky.post('https://api.losos.ayarayarovich.tech/api/dish', {
        json: {
                offset: 0,
                limit: 999999,
                min_price: 0,
...
useIntersectionObserver(containerEl, ([{isIntersecting}]) => {
    if (isIntersecting) {
