          ...
          <div class="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <HeadlessDialogPanel class="w-full max-w-2xl rounded-2xl shadow-xl transition-all">
                <div
                  ref="dialogPanelEl"
                  class="w-full rounded-2xl bg-whitegray pb-8 text-start"
                >
                  <HeadlessDialogTitle
                    class="mb-4 flex items-start justify-between border-b border-gray px-8 pb-4 pt-8 text-xl font-bold"
                  >
...
const { data } = useQuery({
  ...
  queryFn: async () => {
    const response = await publicAxios.get('api/offert')
